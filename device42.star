...
DEVICE42_HOST     = 'swaggerdemo.device42.com' #replace with your Device42fqdn
...
def build_network_interfaces(mac_entries, ip_entries):
    interfaces = []
    # 1) One interface per IP entry (with its macaddress)
    seen_macs = {}
    for ip_obj in ip_entries:
        ...
        # record that we've seen this MAC in an IP record
        seen_macs[macaddr] = seen_macs.get(macaddr, [])
        seen_macs[macaddr].append(ip_str)
        # emit a single‐IP interface now
        interfaces.append(build_network_interface([ip_str], macaddr))

    # 2) For any MACs that never appeared in IP list, still emit an empty‐IP interface
...
def build_assets(devices):
    ...
    for d in devices:
        ...
        # everything else as custom attributes
        custom = {}
        for k, v in d.items():
            if k in (
                'id','name',
...
def main(**kwargs):
    user = kwargs['access_key']
    pwd  = kwargs['access_secret']

    offset = 0
    all_devices = []
    while True:
        url = 'https://' + user + ':' + pwd + '@' + DEVICE42_HOST \
            + DEVICE42_ENDPOINT + '?format=json&offset=' + str(offset)
